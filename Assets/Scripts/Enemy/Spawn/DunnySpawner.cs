...
public class DunnySpawner : MonoBehaviour
{
    public MonsterLoader monsterLoader = new MonsterLoader(); // Reference to the MonsterLoader
    public string monsterName = "Dunny"; // Monster name to load
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            ...
            GameObject[] Dunnies = monsterManager.FindMonster("Dunny(Clone)");

            Debug.Log("Dunny length: " + Dunnies.Length);

            foreach (GameObject dunny in Dunnies) // Proper loop for arrays
            {
                monsterManager.DestroyMonster(dunny); // Corrected syntax for Destroy
                Debug.Log($"Destroyed monster: {dunny.name}");
            }
        }
    }
